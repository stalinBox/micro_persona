...
public class ProductorService {
	...
	/**
	 * Buscar socio productor por Id de Ubicación Domicilio. La busqueda se ejecuta sobre una Vista.
	 *
	 * @param Long ubiId
	 * @return List<Persona>, si cumple la condición. Exception, si no cumple.
	 */
		...
		if (socios == null || socios.size() == 0)
		{
			String msg= MyExceptionUtility.buildExceptionJsonString
					( "error.entity_not_exist.message", 
							ubiId.toString(), 
					  this.getClass(),
					  "findSocioProductorByUbiIdDomicilio",
					  EnumTypeExceptions.INFO,
					  EnumCodeExceptions.DATA_NOT_FOUND_DB, messageSource);
					throw new MyNotFoundException(msg);	
		}
	 ...
	 */
	public Persona saveProductor(Persona persona) {
